var T = {
    W: 32,
    H: 32,
    p1: { x: 0, y: 0, c: 0xCD5C5C, h: 5, spd: 1, fx: 1, fy: 0, p: 1},
    p2: { x: 31, y: 31, c: 0x4169E1, h: 5, spd: 1, fx: -1, fy: 0, p: 2},
    obs: [],
    bullets: [],
    timer: null,
    over: false,
    ...
    startTimer: function() {
        this.timer = PS.timerStart(1, this.updateBullets.bind(this));
    },
    ...
    shoot: function(p) {
        this.bullets.push({ x: p.x, y: p.y, dx: p.fx, dy: p.fy, p: p.p});
    },
    ...
    updateBullets: function() {
        for (let i = this.bullets.length - 1; i >= 0; i--) {
            ...
            if (b.x === this.p1.x && b.y === this.p1.y) {
                this.p1.h -= 1;
                this.bullets.splice(i, 1);
                continue;
            }

            if (b.x === this.p2.x && b.y === this.p2.y) {
                this.p2.h -= 1;
                this.bullets.splice(i, 1);
                continue;
            }
