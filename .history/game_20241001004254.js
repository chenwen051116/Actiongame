var T = {
    W: 15,
    H: 15,
    P1: { x: 1, y: 7, color: 0xCD5C5C, health: 5, speed: 1 }, // Player 1 fighter
    P2: { x: 13, y: 7, color: 0x4169E1, health: 5, speed: 1 }, // Player 2 fighter
    obstacles: [],
    powerUps: [],
    projectiles: [],
    gameOver: false,
    ...
    init: function() {
        ...
        this.placeObstacles();
        this.updateBoard();
        PS.statusText("P1: Arrow keys | P2: WASD | Attack: Space/Shift");
    },
    ...
    updateBoard: function() {
        ...
        for (let i = 0; i < this.powerUps.length; i++) {
            ...
            PS.color(power.x, power.y, 0x00FF00);  // Green power-up
        }
    ...
    checkPowerUp: function(player) {
        for (let i = 0; i < this.powerUps.length; i++) {
            ...
            if (power.x === player.x && power.y === player.y) {
                player.health += 1;  // Boost health
                this.powerUps.splice(i, 1);  // Remove power-up
                break;
            }
        }
    },
    ...
    updateProjectiles: function() {
        for (let i = this.projectiles.length - 1; i >= 0; i--) {
            ...
            if (proj.x < 0 || proj.x >= this.W || proj.y < 0 || proj.y >= this.H) {
                this.projectiles.splice(i, 1);  // Remove projectile if off-grid
                continue;
            }
    ...
    handleKeyPress: function(key) {
        ...
        if (key === PS.KEY_SPACE) this.shootProjectile(this.P1, 1, 0);  // Player 1 shoots right
        ...
        if (key === PS.KEY_SHIFT) this.shootProjectile(this.P2, -1, 0);  // Player 2 shoots left
    }
};
