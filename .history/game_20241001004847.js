var T = {
    ...
    P2: { x: 13, y: 7, color: 0x4169E1, health: 5, speed: 1 },
    obstacles: [],
    powerUps: [],
    projectiles: [],
    ...
    updateProjectiles: function() {
        for (let i = this.projectiles.length - 1; i >= 0; i--) {
            ...
            if (proj.x < 0 || proj.x >= this.W || proj.y < 0 || proj.y >= this.H) {
                this.projectiles.splice(i, 1);  // Remove if off-grid
                continue;
            }
    ...
    handleKeyPress: function(key) {
        ...
        PS.debug(key);
        // Player 1 (Arrow keys)
        if (key === PS.KEY_ARROW_UP) this.movePlayer(this.P2, 0, -1);
        if (key === PS.KEY_ARROW_DOWN) this.movePlayer(this.P2, 0, 1);
        if (key === PS.KEY_ARROW_LEFT) this.movePlayer(this.P2, -1, 0);
        if (key === PS.KEY_ARROW_RIGHT) this.movePlayer(this.P2, 1, 0);
        ...
        if (key === 13) this.shootProjectile(this.P2, -1, 0);  // Player 2 shoots left
    }
};
